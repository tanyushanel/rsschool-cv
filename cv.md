## Nelipovich Tatsiana

![Nelipovich Tatsiana](https://sun9-59.userapi.com/c848536/v848536839/b7690/75_BmXTUylA.jpg)

## Personal info

- age 35
- nationality belarussian
- location: Grodno, Belarus
- married

## Contacts

- Mobile: +375295856916 
- Email: <tatyanan@tut.by>
- Skype: tatyananelipovich
- Telegram: tanyusha nel
- Discord: tanyusha.nel
- GitHub (https://github.com/tanyushanel)

## Summary

 I've been interesting in HTML/CSS/JavaScript technologies since I graduated from university. I went through HTML Academy course when it was just founded. I had some attempts of self-aducation until I was suggested to try RS Shools training course. I consider myself self-disciplined and self-motivated person. I try hard to learn new skills every day and to apply my knowledge wherever it's possible. Of course it tooks some time to understand coding process due to lack of experience. My goal is to become a professional front-end developer.

## Education

* 2002-2007 Grodno State University (bacelor's degree as a filologist)
* 2007-2009 postGraduate GRSU (bacelor's degree as a programmer)
* 2019 QA Introduction EPAM (certificate of QA)
* HTML Academy (2010 - now) (account https://htmlacademy.ru/profile/id5122)
* Codeacademy (HTML & JavaScript course) (https://www.codecademy.com/learn)
* Codewars (https://www.codewars.com/users/tanyushanel)
* RS School 2020 (partly) (https://app.rs.school/profile)
* Trainings on YouTube (Vladilen Minin courses, Дмитрий Лаврик courses&streaming, A.Лущенко courses, WebDev с нуля, RsSchool streaming, IT-Kamasutra, Ivan Petrychenko, Web Developer blog, etc.)

## Skills

* HTML
* JS (basics)
* AJAX (basics)
* Joomla, Wordpress
* JQuery (basics)
* npm, webpack (basics)
* Git (basics)
* QA 

## Examples of work

* [tarzania.by](http://tarzania.by)
* [Singolo](https://tanyushanel.github.io/singolo/) 
* [Keyboard](https://tanyushanel.github.io/keyboard/keybrd.html)
* JS tasks in RSSchool:
- [MorseDecoder](https://github.com/tanyushanel/morse-decoder) 
- [HumanReadableNumber](https://github.com/tanyushanel/human-readable-number)
- [Brackets](https://github.com/tanyushanel/brackets)
- [TowelSort](https://github.com/tanyushanel/towel-sort)
- [ReverseInt](https://github.com/tanyushanel/reverse-int)

## English 
 
* Intermediate+ according to Epam online training
* Enough for reading tech documentation, special books and watching movies 

## Personal skills

* Self-disciplined, hard-working, honest, sociable, responsible, persistent.

